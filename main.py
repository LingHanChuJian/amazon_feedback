...
FEEDBACK_API = '{domain}/sp/ajax/feedback'

RE_STAR = r'(\d+)'

FEEDBACK_DATE = '由 {feedback_name} 于 {date} 评价。'


def random_useragent():
    useragent_arr = []
    for item in browsers:
        useragent_arr.extend(browsers[item])
    return random.choice(useragent_arr)
...
def url_param(url):
    return parse.parse_qs(url_parse(url).query)
...
def get_data(cur_data):
    return ''.join(cur_data).strip().replace('\n', '')


NEED = ('review', 'date', 'star')
NEED_DOC = ('评论', '日期', '星星')
FILE_NAME = 'amazon_feedback_%Y_%m_%d_%H_%M.csv'
...
class JsonCsv:
    ...
    @staticmethod
    def get_path():
        name = time.strftime(FILE_NAME, time.localtime())
        return os.path.join(get_desktop_path(), name)

    def writer_csv(self, dic_data):
        print('正在写入数据')
        for dic in dic_data:
            row = []
            for item in NEED:
...
class Dispose:
    ...
    def dispose(self):
        if type(self.data) == str or type(self.data) == bytes:
            return self.html_dispose(self.get_selector(self.data))
        else:
            return self.json_dispose(self.data)

    def html_dispose(self, selector):
        dispose_data = []
        row = selector.xpath('//tr[@class="feedback-row"]')
        for item in row:
            review = item.xpath('td/div/div[1]//text()')
            date = item.xpath('td/div/div[2]//text()')
            star = item.xpath('th/div/i/@class')
            data_row = {'review': get_data(review), 'date': get_data(date), 'star': self.get_star(star)}
            dispose_data.append(data_row)
        return dispose_data

    @staticmethod
    def json_dispose(json_data):
        dispose_data = []
        if 'details' in json_data:
            for item in json_data['details']:
                review = item['ratingData']['text']['expandedText']
                date = FEEDBACK_DATE.format(feedback_name=item['rater'], date=item['ratingData']['date'])
                star = item['rating']
                dispose_data.append({'review': review, 'date': date, 'star': star})
        return dispose_data
    ...
    @staticmethod
    def get_star(cur_data):
        star = re.search(RE_STAR, get_data(cur_data))
        return star.group(1) if star else ''
...
class Main:
    ...
    def main(self):
        if self.page == 0:
            ...
            wait()
            analysis_url = url_parse(self.url)
            domain = '{protocol}://{host}'.format(protocol=analysis_url.scheme, host=analysis_url.hostname)
            analysis_url_param = url_param(self.url)
            self.marketplace_id = get_data(analysis_url_param.get('marketplaceID', ''))
            self.seller = get_data(analysis_url_param.get('seller', ''))
            if not self.marketplace_id or not self.seller:
                return print('链接没有marketplace_id') if not self.marketplace_id else print('链接没有seller')
            response = self.post_amazon_data(FEEDBACK_API.format(domain=domain), data=self.get_feedback_post_data())
            response = request_message(response, 'json')
            feedback_dispose = Dispose(response)
            feedback_data = feedback_dispose.dispose()
            if feedback_data:
                self.csv.writer_csv(feedback_data)
            else:
                print('没有数据了')
                return self.csv.close_csv()
        self.main()

    def get_feedback_data(self):
        feedback_response = request_message(self.get_amazon_data(), 'txt')
        feedback_dispose = Dispose(feedback_response)
        if is_robot(feedback_dispose.get_selector(feedback_response)):
            return print('机器人验证')
        self.csv.writer_csv(feedback_dispose.dispose())

    def get_feedback_post_data(self):
        return {'seller': self.seller, 'marketplaceID': self.marketplace_id, 'pageNumber': self.page}

    def set_page(self):
        self.page += 1

    def get_amazon_data(self):
        headers = GET_FEEDBACK_HEADERS.copy()
        headers['user-agent'] = self.useragent
        response = self.session.get(url=self.url, headers=headers, timeout=20)
        response.encoding = 'utf-8'
        return response

    def post_amazon_data(self, post_url, data):
        headers = POST_FEEDBACK_HEADERS.copy()
        headers['user-agent'] = self.useragent
        response = self.session.post(url=post_url, data=data, headers=headers, timeout=20)
        response.encoding = 'utf-8'
        return response
...
if __name__ == '__main__':
    while True:
        url = input('请输入爬取amazon seller链接: ')
        if not url:
            print('请输入爬取链接')
        else:
            break
    feedback_main = Main(url)
    feedback_main.main()
